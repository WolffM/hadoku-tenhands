...
- **Fix**: `CRIMSON_CLAUDE_BIN=C:\Users\Hadoku\AppData\Roaming\npm\claude.cmd`
  in `ecosystem.config.cjs` for both Flask + worker.
...
- **Lesson**: cross-repo contracts decay. Add defense in depth
  ...
  `/mnt/c/.../hadoku-aggregator/docs/VIBEDISPATCH-AUDIT-2026-04-18.md`.
...
**Cleaned up**:
- All 9 attempt-#2 forks deleted (WolffM/core, pnpm, express, ollama,
  airflow, argo-cd, biome, fastapi, winget-cli)
...
- **Attempt #3 dispatch** — ready to go once worker restart completes.
  The 10 targets are locked in at
  `state/crimson-kitty/phase4-targets-v2.md` (same as attempt #2,
  minus the already-aborted llama.cpp which will just abort again at
