...
| `temporal/activities/submission.py` | `render_pr_body`, `replicate_fix_as_operator`, `submit_upstream_pr`. `replicate_fix_as_operator` squashes the agent's final tree into a single operator-authored commit on a fresh branch, opens a fork-internal preview PR, and closes the agent's draft PR. The commit has no parent lineage back to agent commits. `submit_upstream_pr` reads the fork preview PR's LIVE title + body via `gh api` (NOT the rendered evidence files) so any edits the operator made between submittable gates passing and the signoff signal flow upstream verbatim, and re-runs `scan_outputs` on that live content as a defense against the operator pasting an upstream URL while editing. See [state-machine.md#awaiting_signoff](state-machine.md) and the "Aggregator convention signal" TODO below. |
| `temporal/activities/watchers.py` | Both watchers wired into `temporal_activities.MAIN_ACTIVITIES` as of Phase 5.1: (1) `notify_human_comments_for_issue` polls upstream issue comments, fires Discord on each new human comment, appends `events.jsonl`; (2) `watch_upstream_pr_state` polls `/pulls/{N}` + `/pulls/{N}/reviews`, returns `{state, merged, closed_unmerged, new_blocking_review, all_seen_review_ids}`, writes terminal `11-merged/` or `11-closed_by_upstream/` evidence, fires Discord ONLY on blocking reviews. The post-submission poll loop in `IssueWorkflow.run` calls both every cadence — comments are side-effect, watch_upstream_pr_state drives state transitions. |
