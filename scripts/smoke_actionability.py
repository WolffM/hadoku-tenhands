...
def _fetch_msft_sso() -> str | None:
    """Fetch MSFT_SSO from the vault using vibedispatch's service-tier key."""
    try:
        vkey = json.loads((REPO_ROOT / ".devvault.local.json").read_text())["key"]
        req = urllib.request.Request(
            "https://hadoku.me/mgmt/api/secrets/get/VIBEDISPATCH_MSFT_SSO",
            headers={"X-User-Key": vkey, "User-Agent": "curl/8.5.0"},
        )
        body = json.loads(urllib.request.urlopen(req, timeout=15).read())
        return body.get("value")
    except Exception as e:
        print(f"  ! could not fetch MSFT_SSO: {e}", file=sys.stderr)
        return None
...
def _gh(args: list[str], saml_org: bool = False) -> dict | list | None:
    ...
    When `saml_org=True`, injects GH_TOKEN=MSFT_SSO so SAML-required orgs
