...
_FIX_INSTRUCTION = (
    ...
    "REQUIRED: also commit a single-line file at exactly this path:\n\n"
    "  05-fixed/test_command.txt\n\n"
    "containing the EXACT shell command that runs the test you wrote in the "
    "repro phase, from the repo root. One line, nothing else. Examples of "
    "valid shapes:\n\n"
    "  pytest tests/test_csv_converter.py -v\n"
    "  go test ./middleware/logger/... -run TestBytesSent -v\n"
    "  pnpm test -- src/components/foo.test.ts\n"
    "  cargo test -p workspace_member specific_test_name -- --nocapture\n\n"
    "A downstream pipeline step runs that command in a clean sandbox and "
    "captures the output, so don't worry about redirecting stdout yourself. "
    "Just write the command. The pipeline does the rest.\n\n"
    "ALSO REQUIRED: commit a file at exactly this path:\n\n"
    "  05-fixed/fix_summary.md\n\n"
    "containing 1-3 short paragraphs in plain prose describing WHAT YOU "
    "CHANGED IN THE CODE and WHY it resolves the issue. This is NOT a "
    "restatement of the bug — the PR body already has that. This is the "
    "fix description a reviewer needs to understand the patch. Do NOT "
    "list files (the diff already shows that). Do NOT write 'the diff is "
    "small enough to read in full' or any equivalent hand-wave — be "
    "concrete about the code change. Example shape:\n\n"
    "  In `parser.py`, the merged-cell resolver was returning `None` when\n"
    "  row 0 had a span > 1 because the anchor-cell lookup walked one row\n"
    "  too far. Clamped the walk to `max(0, anchor_row)` so the lookup\n"
    "  stays inside the merged range. Added a regression test covering a\n"
    "  single-cell header row, a multi-row merge, and the previously-broken\n"
    "  span > 1 case.\n\n"
    "The submission_judge gate reads this directly — vague summaries "
    "(file lists, 'see the diff', 'fixed it') get deferred or rejected."
)

_VERIFY_INSTRUCTION = (
    "Verify your fix conceptually. Re-read the fix you committed and confirm "
    "it actually addresses the root cause described in the upstream issue. "
    "Look for: (1) edge cases the fix might miss, (2) regressions the fix "
    "could introduce in adjacent code paths, (3) whether the test you wrote "
    "actually exercises the corrected behavior end-to-end (not just the "
    "happy path).\n\n"
    "If you find a problem during this re-read, push an additional commit "
    "with the correction. If everything looks right, no action needed — "
    "the pipeline runs your test in a sandbox separately and captures the "
    "output for the verification screenshot.\n\n"
    "Do NOT run the test yourself. Do NOT capture stdout. Do NOT commit a "
    "test_output.txt or after.png — the pipeline handles all of that. Your "
    "only job in this phase is the conceptual re-read."
)
